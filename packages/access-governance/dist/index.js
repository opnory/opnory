...
// ============================================================================
export class LocalGovernanceProvider {
    authority = "local";
    approvalStore;
    auditStore;
    approvalService;
    constructor(approvalStore, auditStore) {
...
export class EntraGovernanceProvider {
    authority = "entra";
    config;
    accessToken = null;
    tokenExpiresAt = 0;
    ...
    async resolveSubject(identity) {
        ...
        try {
            const users = await this.graphRequest(`/users?$filter=mail eq '${identity.requesterEmail}' or userPrincipalName eq '${identity.requesterEmail}'`);
    ...
    async submitRequest(request) {
        ...
        try {
            const result = await this.graphRequest("/identityGovernance/entitlementManagement/assignmentRequests", {
                method: "POST",
    ...
    async getRequestStatus(externalRequestId) {
        try {
            ...
            const result = await this.graphRequest(`/identityGovernance/entitlementManagement/assignmentRequests/${externalRequestId}?$expand=assignment`);
    ...
    async getAssignment(subject, entitlement) {
        try {
            ...
            const assignments = await this.graphRequest(`/identityGovernance/entitlementManagement/assignments?$filter=targetId eq '${subject.id}' and accessPackageId eq '${entitlement.externalId}'`);
    ...
    async revokeAssignment(assignment) {
        try {
            ...
            await this.graphRequest("/identityGovernance/entitlementManagement/assignmentRequests", {
                method: "POST",
                body: JSON.stringify({
...
export class OktaGovernanceProvider {
    authority = "okta";
    config;
    accessToken = null;
    tokenExpiresAt = 0;
    ...
    async generateJwtAssertion(scopes) {
        const { SignJWT } = await import("jose");
        // Parse PEM private key
        const privateKey = await import("jose").then(({ importPKCS8 }) => importPKCS8(this.config.privateKey, "RS256"));
    ...
    async oktaRequest(path, options = {}) {
        const token = await this.getAccessToken();
    ...
    async resolveSubject(identity) {
        ...
        try {
            const users = await this.oktaRequest(`/api/v1/users?filter=profile.email eq "${identity.requesterEmail}"&limit=1`);
            if (users.length > 0) {
                const user = users[0];
                return GovernanceSubjectSchema.parse({
                    id: user.id,
    ...
    async submitRequest(request) {
        ...
        try {
            const result = await this.oktaRequest("/governance/api/v2/requests", {
                method: "POST",
    ...
    async getRequestStatus(externalRequestId) {
        try {
            ...
            const result = await this.oktaRequest(`/governance/api/v2/requests/${externalRequestId}`);
    ...
    mapOktaRequestStatus(status, state) {
...
export function createGovernanceProvider(authority, config = {}) {
    switch (authority) {
        case "local":
...
export class GovernanceService {
    ...
    providers = new Map();
    auditStore;
    constructor(localProvider, entraProvider, oktaProvider, auditStore) {
        this.localProvider = localProvider;
        this.entraProvider = entraProvider;
        this.oktaProvider = oktaProvider;
    ...
    async submitGovernedRequest(request, entitlement, correlationId) {
        ...
        const governedRequest = GovernedAccessRequestSchema.parse({
            requestId: request.id,
            subject,
            entitlement: governedEntitlement,
            justification: request.reason,
            requestedDuration: request.metadata?.requestedDuration,
        });
    ...
    async checkGovernanceStatus(request) {
    ...
    async reconcileGovernance(request) {
        if (!request.governanceExternalRequestId || !request.governanceAuthority) {
            return;
        }
        const provider = this.getProvider(request.governanceAuthority);
...
// ============================================================================
export class GovernanceReconcilerImpl {
    governanceService;
    config;
    auditStore;
    approvalStore;
    constructor(governanceService, config = {}, auditStore, approvalStore) {
    ...
    async recordReconciliationAudit(type, metadata) {
        await recordAuditEvent(this.auditStore, {
            eventId: crypto.randomUUID(),
    ...
    async getPendingRequests() {
        ...
        return allRequests.filter((r) => r.governanceExternalRequestId &&
            r.governanceAuthority &&
            r.status === "AWAITING_AUTHORITY_DECISION");
    }
    async getActiveAssignments() {
        const allRequests = await this.approvalStore.getAll();
        return allRequests.filter((r) => r.governanceAssignmentId &&
            r.governanceAuthority &&
            r.status === "FULFILLED");
    }
    ...
    async reconcilePendingRequests() {
        ...
        for (const request of pendingRequests) {
            ...
            try {
                ...
                request.governanceNextCheckAt = new Date(Date.now() + this.config.retryBackoffMs).toISOString();
                ...
                if (request.governanceRetryCount &&
                ...
                logger.warn({ requestId: request.id, error: errorMessage }, "Reconciliation failed for request");
            }
        }
    ...
    async reconcileAssignments() {
        ...
        for (const request of activeAssignments) {
            ...
            try {
                ...
                logger.warn({ requestId: request.id, error: errorMessage }, "Assignment reconciliation failed");
            }
        }
        await this.recordReconciliationAudit("GOVERNANCE_RECONCILIATION_SUCCEEDED", {
            provider: this.config.provider,
    ...
    async reconcileRevocations() {
        ...
        for (const request of pendingRevocations) {
            ...
            try {
                ...
                logger.warn({ requestId: request.id, error: errorMessage }, "Revocation reconciliation failed");
            }
        }
        await this.recordReconciliationAudit("GOVERNANCE_RECONCILIATION_SUCCEEDED", {
            provider: this.config.provider,
