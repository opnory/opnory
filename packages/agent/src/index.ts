...
export class OpnoryAgent {
  ...
  async processRequest(request: NormalizedRequest): Promise<AgentResponse> {
    ...
    traceLogger.info(
      { step: "normalize", correlationId },
      "Processing request",
    );

    // Step 1: Retrieve relevant knowledge
    traceLogger.info(
      { step: "retrieve", query: request.text, correlationId },
    ...
    traceLogger.debug(
      { count: searchResults.length, correlationId },
    ...
    traceLogger.debug(
      { contextLength: context.length, correlationId },
      "Built context",
    );
    ...
    const agentOutput = await this.generateResponse(
      request,
      context,
      searchResults,
      correlationId,
    ...
    if (agentOutput.status === "escalated") {
      traceLogger.info(
        { reason: agentOutput.escalationReason, correlationId },
...
Matched Excerpts: ${excerpts}`;
  ...
  private async generateResponse(
    ...
    if (searchResults.length === 0) {
      return {
        status: "escalated",
        answer:
          "I couldn't find any relevant information in the company knowledge base to answer your question. This requires human support.",
        citations: [],
        confidence: 0.1,
        escalationId: undefined,
    ...
    if (topResult.score < 0.3) {
      return {
        status: "escalated",
        answer:
          "I found some potentially related documents, but they don't contain enough information to confidently answer your question. Escalating to human support.",
        citations: [],
    ...
    if (!hasCompleteAnswer) {
      return this.buildPartialResponse(
        request.text,
        searchResults,
        correlationId,
      );
    ...
    return this.buildGroundedResponse(
      request.text,
      searchResults,
      correlationId,
    );
  ...
  private buildPartialResponse(
    ...
    const confidence = Math.min(
      topResult.score * 0.8 + (searchResults.length > 1 ? 0.1 : 0),
      0.9,
    );
  ...
  private buildGroundedResponse(
    ...
    const confidence = Math.min(
      topResult.score * 0.85 + (searchResults.length > 1 ? 0.1 : 0),
      0.95,
    );
...
export function createAgent(deps?: Partial<AgentDependencies>): OpnoryAgent {
  ...
  const escalationService =
    deps?.escalationService ?? new InMemoryEscalationService(escalationStore);
