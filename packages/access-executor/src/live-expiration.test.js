...
import { toFulfilledAccessRequest } from "@opnory/access-types";
import { Pool } from "pg";
...
describe.skipIf(!process.env.OPNORY_LIVE_GITHUB_TESTS)("ExpirationScheduler - Live Accelerated Test", () => {
    let executor;
    let auditStore;
    let pgPool;
    let scheduler;
    ...
    it("TEST 1: should grant access then expire it within test TTL", async () => {
        ...
        const finalResult = await pgPool.query("SELECT status FROM access_requests WHERE id = $1", [testRequestId]);
    ...
    it("TEST 2: should be idempotent - second run does nothing", async () => {
        ...
        const result = await pgPool.query("SELECT status FROM access_requests WHERE id = $1", [testRequestId]);
    ...
    it("TEST 3: extension protection - extended accessExpiresAt prevents revocation", async () => {
        ...
        const result = await pgPool.query("SELECT status, access_expires_at FROM access_requests WHERE id = $1", [testRequestId2]);
...
//# sourceMappingURL=live-expiration.test.js.map