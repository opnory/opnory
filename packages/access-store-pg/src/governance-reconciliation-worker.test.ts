...
function createMockReconciliationProvider(): GovernanceProvider & {
  ...
  function trackCall(
    ...
    method: "getRequestStatus" | "getAssignment" | "revokeAssignment",
  ) {
    const counts = callCounts.get(externalRequestId) || {
      getRequestStatus: 0,
      getAssignment: 0,
      revokeAssignment: 0,
    };
  ...
  const provider: GovernanceProvider & {
    ...
    async submitRequest(
      ...
      callCounts.set(externalRequestId, {
        getRequestStatus: 0,
        getAssignment: 0,
        revokeAssignment: 0,
      });
    ...
    callsFor(externalRequestId: string) {
      return (
        callCounts.get(externalRequestId) || {
          getRequestStatus: 0,
          getAssignment: 0,
          revokeAssignment: 0,
        }
...
describe("Governance Reconciliation Worker Conformance (CASE 65-72)", () => {
  ...
  beforeEach(async () => {
    ...
    const mockReconciler = {
      getProvider: (authority: string) =>
        authority === "entra" ? provider : null,
    } as any;
  ...
  async function insertTestRequest(
    ...
    const nextCheckAt =
      overrides.governanceNextCheckAt || new Date(now.getTime() - 1000);
    const nextAttemptAt =
      overrides.governanceNextAttemptAt || new Date(now.getTime() - 1000);
    ...
    `,
      [
        ...
        JSON.stringify({
          governance: {
            provider: overrides.governanceAuthority || "entra",
  ...
  describe("CASE 65 — Two workers race for same governance request", () => {
    it("should only make one provider call when two workers race", async () => {
      ...
      // Run both workers
      await Promise.all([worker1.runOnce(), worker2.runOnce()]);
      ...
      const result = await pool.query(
        "SELECT status, governance_assignment_id FROM access_requests WHERE id = $1",
        [id],
      );
      ...
      const succeededEvents = events.filter(
        (e: any) => e.type === "GOVERNANCE_RECONCILIATION_SUCCEEDED",
      );
  ...
  describe("CASE 66 — Active foreign lease → zero processing", () => {
    it("should skip requests leased by another worker", async () => {
      ...
      const events = await auditStore.getByRequestId(id);
      const reconciliationEvents = events.filter((e: any) =>
        e.type.startsWith("GOVERNANCE_RECONCILIATION"),
      );
  ...
  describe("CASE 67 — Expired lease → reclaim and continue", () => {
    it("should reclaim and process requests with expired leases", async () => {
      ...
      expect(result.rows[0].status).toBe("APPROVED");
      expect(result.rows[0].governance_assignment_id).toBe(
        "assignment-expired-1",
      );
  ...
  describe("CASE 68 — Provider timeout → retry metadata + backoff", () => {
    it("should schedule retry with backoff on provider timeout", async () => {
      ...
      const mockReconciler = {
        getProvider: (authority: string) =>
          authority === "entra" ? timeoutProvider : null,
      } as any;
      ...
      const failedEvents = events.filter(
        (e: any) => e.type === "GOVERNANCE_RECONCILIATION_FAILED",
      );
  ...
  describe("CASE 70 — Request becomes local before processing → skip", () => {
    it("should skip requests that are no longer externally governed", async () => {
      ...
      const startedEvents = events.filter(
        (e: any) => e.type === "GOVERNANCE_RECONCILIATION_STARTED",
      );
