...
import { ExpirationScheduler, createExpirationScheduler } from "./expiration-scheduler.js";
import { GitHubAccessExecutor, InMemoryIdempotencyStore } from "@opnory/access-executor";
...
class MockExecutor {
    ...
    async grant(request) {
        ...
        if (this.shouldFail) {
            ...
            err.status = this.failStatusCode;
...
async function runExpirationTick(scheduler) {
    // Use reflection to call private method
    await scheduler.processDueExpirations();
}
// ============================================================================
...
describe("ExpirationScheduler - Core Lifecycle (CASE 22-29)", () => {
    ...
    it("CASE 28A — Extension committed before claim: scheduler sees T2, skips", async () => {
        ...
        await testPool.query(`UPDATE access_requests SET access_expires_at = $1, version = version + 1, updated_at = NOW() WHERE id = $2`, [T2.toISOString(), request.id]);
...
describe("Distributed Expiration Execution (CASE 30-40)", () => {
    ...
    it("CASE 31 — Expired lease is reclaimable by another worker", async () => {
        ...
        await testPool.query(`UPDATE access_requests SET 
    ...
    it("CASE 35 — Extension before claim prevents revocation", async () => {
        ...
        version = version + 1,
        updated_at = NOW()
      WHERE id = $2`, [T2.toISOString(), request.id]);
...
describe("ExpirationScheduler - Unit Tests (No DB)", () => {
    ...
    beforeAll(() => {
        ...
        }, new InMemoryIdempotencyStore(), auditStore);
    });
    beforeEach(() => {
        mockPool = {
    ...
    it("should have processDueExpirations method", async () => {
        ...
        expect(typeof scheduler.processDueExpirations).toBe("function");
    });
...
//# sourceMappingURL=expiration-scheduler.test.js.map