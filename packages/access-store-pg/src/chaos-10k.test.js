...
import { createExpirationScheduler, DEFAULT_SCHEDULER_CONFIG } from "./expiration-scheduler.js";
...
async function insertRequest(pool, request) {
    await pool.query(`INSERT INTO access_requests (
...
async function getRequestStatus(pool, id) {
...
async function getAuditEvents(pool, requestId) {
    const result = await pool.query("SELECT * FROM audit_events WHERE request_id = $1 ORDER BY timestamp", [requestId]);
    return result.rows;
}
async function classifyResults(pool) {
    const result = {
    ...
    const leaseCheck = await pool.query(`SELECT id, lease_owner, lease_until FROM access_requests 
     WHERE id = ANY($1) AND lease_owner IS NOT NULL AND lease_until > NOW() AND status NOT IN ('REVOKED', 'REVOCATION_FAILED')`, [seededIds]);
    result.leaseConflicts = leaseCheck.rows.length;
    return result;
}
function printClassification(result, phase) {
    console.log(`\n═══════════════════════════════════════════`);
...
describe.skip("10K Chaos / Recovery Validation", () => {
    beforeAll(async () => {
        ...
        testScheduler = createExpirationScheduler(chaosExecutor, new PgAuditEventStore(testPool), testPool, config);
    });
    afterAll(async () => {
        await testPool.end();
    });
    it("Full 10K Chaos / Recovery Validation", async () => {
        ...
        for (let round = 1; round <= 150; round++) {
            ...
            if (results.claimsThisPoll === 0) {
                ...
                const retryCheck = await testPool.query(`SELECT COUNT(*) FROM access_requests WHERE reason = 'Chaos test expiration' AND status = 'RETRY' AND expiration_next_attempt_at <= NOW()`);
        ...
        const convergedTotal = recoveryClassification.convergedImmediately.revoked +
...
//# sourceMappingURL=chaos-10k.test.js.map