...
class MockExecutor {
  ...
  async revoke(request: AccessRequest): Promise<RevocationResult> {
    ...
    if (this.shouldFail) {
      ...
      call.result = {
        success: false,
        error: `Mock failure (${this.failStatusCode})`,
...
async function cleanupTestData(pool: Pool) {
  await pool.query(
    `DELETE FROM access_requests WHERE requester_id LIKE 'test-%' OR requester_id LIKE 'case-%'`,
  );
  await pool.query(
    `DELETE FROM audit_events WHERE request_id IN (SELECT id FROM access_requests WHERE requester_id LIKE 'test-%' OR requester_id LIKE 'case-%')`,
  );
}
...
beforeEach(async () => {
  ...
  testScheduler = new ExpirationScheduler(
    testExecutor,
    testAuditStore,
    testPool,
    {
...
describe("ExpirationScheduler - Core Lifecycle (CASE 22-29)", () => {
  ...
  it("CASE 24 — Restart recovery: expires while worker offline → startup scan discovers", async () => {
    ...
    const restartedScheduler = new ExpirationScheduler(
      testExecutor,
      testAuditStore,
      testPool,
      {
  ...
  it("CASE 25 — Concurrent workers: two workers claim same due request → one winner", async () => {
    ...
    const schedulerA = new ExpirationScheduler(
      testExecutor,
      testAuditStore,
      testPool,
      {
    ...
    const schedulerB = new ExpirationScheduler(
      testExecutor,
      testAuditStore,
      testPool,
      {
  ...
  it("CASE 28A — Extension committed before claim: scheduler sees T2, skips", async () => {
    ...
    const updated = await getRequest(testPool, request.id);
    expect(updated.status).toBe("FULFILLED");
    expect(new Date(updated.access_expires_at).getTime()).toBeCloseTo(
      T2.getTime(),
      -3,
    ); // Within 1s
