...
// ============================================================================
function createMockReconciliationProvider() {
    const requests = new Map();
    const assignments = new Map();
    let requestCounter = 0;
    // Request-specific call tracking
    const callCounts = new Map();
    function trackCall(externalRequestId, method) {
    ...
    const provider = {
        ...
        preSetAssignmentStatus(assignmentId, status) {
            for (const [key, assignment] of assignments.entries()) {
                if (assignment.assignmentId === assignmentId) {
...
describe("Governance Reconciliation Worker Conformance (CASE 65-72)", () => {
    ...
    `);
        ...
        const mockReconciler = {
            getProvider: (authority) => authority === "entra" ? provider : null,
        };
        worker = new GovernanceReconciliationWorker(mockReconciler, auditStore, pool, {
            pollIntervalMs: 100,
            pollJitterMs: 0,
    ...
    describe("CASE 65 — Two workers race for same governance request", () => {
        it("should only make one provider call when two workers race", async () => {
            ...
            const worker1 = new GovernanceReconciliationWorker(mockReconciler, auditStore, pool, { pollIntervalMs: 100, pollJitterMs: 0, batchSize: 1, providerConcurrency: 1, leaseDurationMs: 5000 });
            const worker2 = new GovernanceReconciliationWorker(mockReconciler, auditStore, pool, { pollIntervalMs: 100, pollJitterMs: 0, batchSize: 1, providerConcurrency: 1, leaseDurationMs: 5000 });
            // Run both workers
            await Promise.all([
                worker1.runOnce(),
                worker2.runOnce(),
            ...
            const succeededEvents = events.filter((e) => e.type === "GOVERNANCE_RECONCILIATION_SUCCEEDED");
    ...
    describe("CASE 66 — Active foreign lease → zero processing", () => {
      ...
      `, [new Date(Date.now() + 60000), id]);
            ...
            const events = await auditStore.getByRequestId(id);
            const reconciliationEvents = events.filter((e) => e.type.startsWith("GOVERNANCE_RECONCILIATION"));
    ...
    describe("CASE 68 — Provider timeout → retry metadata + backoff", () => {
        it("should schedule retry with backoff on provider timeout", async () => {
            ...
            const mockReconciler = {
                getProvider: (authority) => authority === "entra" ? timeoutProvider : null,
            };
            const timeoutWorker = new GovernanceReconciliationWorker(mockReconciler, auditStore, pool, {
                pollIntervalMs: 100,
                pollJitterMs: 0,
                batchSize: 1,
            ...
            const result = await pool.query("SELECT status, governance_attempt_count, governance_next_attempt_at FROM access_requests WHERE id = $1", [id]);
            ...
            const events = await auditStore.getByRequestId(id);
            const failedEvents = events.filter((e) => e.type === "GOVERNANCE_RECONCILIATION_FAILED");
    ...
    describe("CASE 69 — Restart finds overdue reconciliation", () => {
        it("should process overdue reconciliations on worker restart", async () => {
            ...
            const newWorker = new GovernanceReconciliationWorker(mockReconciler, auditStore, pool, { pollIntervalMs: 100, pollJitterMs: 0, batchSize: 1, providerConcurrency: 1, leaseDurationMs: 5000 });
    ...
    describe("CASE 70 — Request becomes local before processing → skip", () => {
      ...
      `, [JSON.stringify({ governance: { provider: "local" } }), id]);
            ...
            const events = await auditStore.getByRequestId(id);
            const reconciliationEvents = events.filter((e) => e.type.startsWith("GOVERNANCE_RECONCILIATION"));
            const startedEvents = events.filter((e) => e.type === "GOVERNANCE_RECONCILIATION_STARTED");
            const completedEvents = events.filter((e) => e.type === "GOVERNANCE_RECONCILIATION_SUCCEEDED" ||
    ...
    describe("CASE 72 — Drift reconciliation runs once under concurrent workers", () => {
        it("should detect drift only once when two workers race on FULFILLED assignment", async () => {
            ...
            const events = await auditStore.getByRequestId(id);
            const driftEvents = events.filter((e) => e.type === "GOVERNANCE_DRIFT_DETECTED");
            expect(driftEvents.length).toBe(1);
            // Should only correct state once
            const correctionEvents = events.filter((e) => e.type === "GOVERNANCE_STATE_CORRECTED");
            expect(correctionEvents.length).toBe(1);
        });
    });
});
//# sourceMappingURL=governance-reconciliation-worker.test.js.map