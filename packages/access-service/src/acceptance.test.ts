...
class FakeEntraGovernanceProvider implements GovernanceProvider {
  ...
  registerAccessPackage(accessPackageId: string) {
  ...
  private mapEntraStatus(entraStatus: string): GovernanceRequest["status"] {
    switch (entraStatus) {
...
// ============================================================================
// Fake Okta Governance Provider (in-memory simulation)
// ============================================================================

interface FakeOktaRequest {
  id: string;
  groupId: string;
  subjectId: string;
  requestStatus: string;
...
class FakeOktaGovernanceProvider implements GovernanceProvider {
  ...
  async submitRequest(
    ...
    const fakeRequest: FakeOktaRequest = {
      id: externalRequestId,
  ...
  async getRequestStatus(
    ...
    return {
      externalRequestId,
      status: request.requestStatus as GovernanceRequestStatus["status"],
      assignmentId: request.assignmentId,
      lastPolledAt: new Date().toISOString(),
      rawResponse: request,
    };
  ...
  private mapOktaStatus(oktaStatus: string): GovernanceRequest["status"] {
    switch (oktaStatus) {
...
// Type for EntitlementRef in this test file
type EntitlementRef = {
  id: string;
  name: string;
  system: string;
  ...
  governance?: {
    provider: "local" | "entra" | "okta";
    authority?: GovernanceAuthority;
    fulfillmentOwner?: string;
    durationOwner?: string;
    // Entra
    accessPackageId?: string;
...
describe("Access Request Service - Acceptance Tests", () => {
  ...
  let executor: FakeGitHubAccessExecutor;
