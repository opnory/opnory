...
|---------|----------------------------------------|
| `fastify({ trustProxy: true })` | `fastify({ trustProxy: ["127.0.0.1", "<CADDY_COMPOSE_SUBNET>/<PREFIX>"] })` — exact Caddy source IP/subnet (the Docker compose network CIDR for the Gate 3 API stack, determined at deploy time) only |

**Rationale**: The Fastify advisory specifically recommends an IP/CIDR/custom trust predicate that validates the connecting address and ensures the origin cannot be reached around the proxy. Trusting all RFC1918 space (`10/8`, `172.16/12`, `192.168/16`) is **too broad** for a hardened deployment. Only the **actual Caddy source IP/subnet** (the Docker compose network CIDR used for the Gate 3 API stack, e.g., `172.20.0.0/16` or the exact subnet at deploy time) should be trusted. This is the minimal trust boundary that satisfies the advisory while maintaining security.
...
*All D-1–D-6 decisions locked 2026-09-13 by user. No implementation performed. Gate 2 SHA unchanged.*