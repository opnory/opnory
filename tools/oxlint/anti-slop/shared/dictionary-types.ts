...
function resolvesToDictionary(
  ...
  if (unwrapped.type === "TSTypeLiteral" || unwrapped.type === "TSMappedType") return true;
  ...
  if (substitution !== undefined) {
    return isUnappliedReferenceTo(substitution, name)
      ? false
      : resolvesToDictionary(substitution, environment, substitutions, resolvingAliases);
  }
  ...
  if (TRANSPARENT_WRAPPERS.has(name) && isBuiltIn(name, environment)) {
    ...
    return wrapped === undefined
      ? false
      : resolvesToDictionary(wrapped, environment, substitutions, resolvingAliases);
  }

  if (name === "Record" && isBuiltIn(name, environment)) return true;

  if ((name === "Pick" || name === "Omit") && isBuiltIn(name, environment)) {
    const source = unwrapped.typeArguments?.params[0];
    return source === undefined
      ? false
      : resolvesToDictionary(source, environment, substitutions, resolvingAliases);
  }
  ...
  const nextResolving = new Set(resolvingAliases);
  nextResolving.add(name);
  return resolvesToDictionary(alias.typeAnnotation, environment, nextSubstitutions, nextResolving);
}
...
export function classifyWideningTarget(
  ...
  const substitution = new Map().get(name);
  if (substitution !== undefined) {
    return isUnappliedReferenceTo(substitution, name)
      ? null
      : classifyAliasBroadTarget(substitution, environment, new Map(), new Set());
  }
  ...
  if (TRANSPARENT_WRAPPERS.has(name) && isBuiltIn(name, environment)) {
    ...
    return wrapped === undefined
      ? null
      : classifyAliasBroadTarget(wrapped, environment, new Map(), new Set());
  }
  if (name === "Record" && isBuiltIn(name, environment)) {
  ...
  const nextSubstitutions = aliasSubstitution(alias, unwrapped, new Map());
  if (nextSubstitutions === null) return null;
