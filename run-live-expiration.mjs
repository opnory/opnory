#!/usr/bin/env node
// Live expiration test - plain JavaScript with dynamic imports
...
// We'll use dynamic imports for our TypeScript modules
const { GitHubAccessExecutor, InMemoryIdempotencyStore } = await import('./packages/access-executor/dist/index.js');
const { InMemoryAuditEventStore } = await import('./packages/access-audit/dist/index.js');
const { ExpirationScheduler } = await import('./packages/access-store-pg/dist/expiration-scheduler.js');
const { toFulfilledAccessRequest } = await import('./packages/access-types/dist/index.js');
...
async function runLiveExpirationTest() {
  ...
  const executor = new GitHubAccessExecutor(
    {
      appId: "4647201",
  ...
  const scheduler = new ExpirationScheduler(executor, auditStore, pgPool, {
